DANHGIAPROBOOTH - Cloudflare Pages + D1 (bản deploy đã kiểm tra)

CẤU TRÚC:
- index.html: giao diện khách + trang quản trị
- functions/api/reviews.js: API đánh giá, rate-limit, Turnstile, đăng nhập admin
...
5. Cấu hình Cloudflare Turnstile: tạo Site Key + Secret Key và đặt TURNSTILE_SITE_KEY + TURNSTILE_SECRET_KEY. Turnstile là bắt buộc để gửi review.
6. Trang quản trị: https://<ten-project>.pages.dev/?admin=1
...
- Rate-limit được kiểm tra ngay trong câu INSERT có điều kiện để tránh tình trạng nhiều request đồng thời cùng vượt qua một COUNT(*) kiểm tra riêng.
...
TURNSTILE:
- Frontend lấy Site Key từ /api/config và hiển thị Turnstile.
- Backend bắt buộc có TURNSTILE_SECRET_KEY và xác minh token trước khi lưu.
...
- Sau deploy, thử gửi 1 đánh giá thật và kiểm tra trong ?admin=1.
- Kiểm tra đăng nhập sai nhiều lần để xác nhận rate-limit admin.
